...
func NewChannel(wc *websocket.Conn, id string, opts ...ChannelOption) *Channel {
...
// SetTopic will update the channel's topic subscription to include the provided topic
...
// RemoveTopic will update the channel's topic subscription to not include the provided topic
func (c *Channel) RemoveTopic(topic string) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// HasTopic will return true or false as to whether or not the channel is subscribed to the topic or not
func (c *Channel) HasTopic(topic string) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Channel) Send(ctx context.Context, msg SendMessage) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	case c.writeBuf <- msg.Message:
	}
	return nil
}
